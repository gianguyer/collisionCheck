...
namespace VMS.TPS
{
    ...
    public class Script
    {
        ...
        //  Run pathCheck on Remote

        void runBlender()
        {
            string cmd = remoteDir + "/code/run-blender " + studyName + " " + patientName + " " + planName + " " + blenderTask;

            runRemoteCommand(remote, cmd);
        }
        ...
        //  Copy blender settings file to Remote

        void copyBlenderSettings()
        {
            string subDir = "studies/" + studyName + "/" + patientName + "/" + planName;
            string remoteSetupDir = remoteDir + "/" + subDir;
            string setupDir = (studyDir).Replace('\\' , '/');
            string remoteConnect = remote + ":" + remoteSetupDir + "/";

            string files = "blender.properties";
			
            runRemoteCommand(remote, "mkdir -p " + remoteSetupDir);

            runBashCommand("cd " + setupDir + "; scp -r " + files + " " + remoteConnect);

            runRemoteCommand(remote, "chmod -R a=r,a+X,u+w " + remoteSetupDir);
        }
		
        //  Copy plan files to Remote
        ...
        void runRemoteCommand(string remote, string cmd)
        {
            runBashCommand("ssh -o ServerAliveInterval=60 -o ServerAliveCountMax=30 " + remote + " " + cmd); 
        }
        ...
        {
            string[] studies = Util.getSubDirs(localDir);
        ...
        void initWindow(ScriptContext context, Window window)
        {
			...
			studyName = "demo";
            studyDir = localDir + "\\" + "demo" + "\\" + patientName + "\\" + planName;
            ...
            studyCombo.SelectionChanged += (s, ev) =>
            {
                ...
                studyDir = localDir + "\\" + studyName + "\\" + patientName + "\\" + planName;
            };
            ...
            ok.Click += (s, ev) =>
            {
                ...
                if (MessageBox.Show("Patient: " + patientName + "\nPlan : " + planName + "\n\nCopy blender settings to Remote ?", "Copy Setup", MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    copyBlenderSettings();
